...
- **AI Services**: OpenAI for response generation and embedding creation
...
2. **Embedding Generator**: Creates vector embeddings for content
...
- Vector size: Based on OpenAI embedding model (1536 for text-embedding-ada-002)
...
pip install fastapi uvicorn python-dotenv openai qdrant-client psycopg2-binary sqlalchemy python-multipart python-jose[cryptography] passlib[bcrypt] python-uuid
...
4. Create OpenAI integration for response generation
